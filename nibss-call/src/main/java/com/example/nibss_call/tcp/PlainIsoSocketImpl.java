package com.interswitchng.smartpos.shared.services.nibss.tcp;
...
public class PlainIsoSocketImpl implements IsoSocket {
    ...
    public PlainIsoSocketImpl(SocketAddress socketAddress, int timeout) {
        this.socketAddress = socketAddress;
        this.timeout = timeout;
    }

    public PlainIsoSocketImpl(String serverIp, int serverPort, int timeout) {
    ...
    public boolean open() {
        try {
            socket = (Socket) factory.createSocket(serverIp, serverPort);
    ...
    public boolean send(@NonNull byte[] data) {
        ...
        System.out.println("+++ plain dataToSend ---> " + IsoUtils.bytesToHex(dataToSend));

        if (this.socket.isConnected()) {
            System.out.println("Plain Socket is connected " + serverIp + ":" + serverPort);
            try {
    ...
    public byte[] receive() {
        ...
        int dataLen;
        try {
            System.out.println("Plain Socket is attempting to receive from " + serverIp + ":" + serverPort);
            ...
            System.out.println("Plain Socket data length:  " + dataLen);
